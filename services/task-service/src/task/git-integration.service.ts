...
export class GitIntegrationService {
  ...
  constructor(
    ...
    @InjectModel('GitIntegrationConfig') private gitConfigModel: Model<IGitIntegrationConfig>,
  ) {}
  ...
  async removeGitConfig(orgId: string, provider?: string) {
    const filter: any = { organizationId: orgId };
    if (provider) filter.provider = provider;
    return this.gitConfigModel.deleteMany(filter);
  }
