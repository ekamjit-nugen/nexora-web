...
export class CallingGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  handleDisconnect(client: Socket) {
    ...
    if (userId) {
      // Notify other participants in any active call this socket was part of
      for (const [callId, sockets] of this.callSessions.entries()) {
        if (sockets.has(client.id)) {
          sockets.delete(client.id);
          // If this user has no other sockets in the call, notify remaining participants
          ...
          if (!userHasOtherSockets) {
            ...
            this.logger.log(`Call ${callId} ended due to user ${userId} disconnect`);
          }
          if (sockets.size === 0) {
  ...
  async handleEndCall(
    ...
    try {
      ...
      this.server.to(`call:${call.callId}`).emit('call:ended', {
        callId: call.callId,
        endedBy: userId,
        duration: call.duration,
      });
      // Also emit directly to users (in case they are not in the room)
