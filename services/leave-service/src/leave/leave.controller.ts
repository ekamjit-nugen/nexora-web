...
export class LeaveController {
  ...
  async applyLeave(@Body() dto: ApplyLeaveDto, @Req() req) {
    const leave = await this.leaveService.applyLeave(dto, req.user.userId, req.user?.organizationId);
  ...
  async approveLeave(@Param('id') id: string, @Body() dto: ApproveLeaveDto, @Req() req) {
    const leave = await this.leaveService.approveLeave(id, dto, req.user.userId, req.user?.organizationId);
  ...
  async cancelLeave(@Param('id') id: string, @Body() dto: CancelLeaveDto, @Req() req) {
    const leave = await this.leaveService.cancelLeave(id, dto, req.user.userId, req.user?.organizationId);
