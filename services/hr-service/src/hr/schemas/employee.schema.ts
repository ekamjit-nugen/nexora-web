...
export const EmployeeSchema = new Schema<IEmployee>(
  {
    ...
    status: {
      type: String,
      // `declined` — invitee explicitly rejected the org invite. Keeps the
      // audit trail visible to admins instead of silently reverting to
      // "invited" (previously the decline flow never touched HR at all,
      // leaving the directory inconsistent with auth membership state).
      enum: ['active', 'invited', 'pending', 'declined', 'on_notice', 'exited', 'on_leave', 'probation'],
      default: 'active',
    },
