...
@Module({ imports: [BootstrapModule, CallingModule] })
class CallingStandaloneModule {}

async function bootstrap() {
  if (!process.env.JWT_SECRET) { console.error('FATAL: JWT_SECRET'); process.exit(1); }
  const log = new Logger('CallingStandalone');
  const app = await NestFactory.create(CallingStandaloneModule);
  ...
  const port = Number(process.env.CALLING_PORT || process.env.PORT || 3051);
  await app.listen(port);
  log.log(`📞 Calling (standalone) listening on http://localhost:${port}`);
}
